...
def interpolate_2d_pos_embed_to_3d(pos_embed_2d, num_patches_3d):
    """
    Interpolate 2D position embeddings for use in a 3D grid, handling the cls token.

    Parameters:
        pos_embed_2d (Tensor): Pretrained 2D position embeddings of shape (1, num_patches_2d + 1, embed_dim).
        num_patches_3d (tuple): Target grid size as (depth, height, width) for 3D input.

    Returns:
        Tensor: Interpolated 3D position embeddings with the cls token.
    """
    # Extract dimensions
    batch_size, num_patches_2d_plus_cls, embed_dim = pos_embed_2d.shape

    # Separate cls token embedding
    cls_token = pos_embed_2d[:, 0:1, :]  # Shape: (1, 1, embed_dim)
    pos_embed_2d = pos_embed_2d[:, 1:, :]  # Shape: (1, num_patches_2d, embed_dim)

    # Calculate grid size for the 2D patches (assuming square for simplicity)
    grid_size_2d = int(np.sqrt(pos_embed_2d.shape[1]))

    # Reshape the 2D position embeddings to (height, width, embed_dim)
    pos_embed_2d = pos_embed_2d.reshape(1, grid_size_2d, grid_size_2d, embed_dim).permute(0, 3, 1, 2)  # (1, embed_dim, height, width)

    # Define target size (depth, height, width)
    target_size = (num_patches_3d[0], num_patches_3d[1], num_patches_3d[2])

    # Interpolate using trilinear interpolation to match target 3D grid
    pos_embed_3d = F.interpolate(
        pos_embed_2d.unsqueeze(2),  # Add a depth dimension (N, C, D, H, W)
        size=target_size,
        mode='trilinear',
        align_corners=False
    ).squeeze(0).permute(1, 2, 3, 0).reshape(-1, embed_dim).unsqueeze(0)  # (1, num_patches_3d[0] * num_patches_3d[1] * num_patches_3d[2], embed_dim)

    # Concatenate the cls token back with the interpolated patch embeddings
    pos_embed_3d = torch.cat((cls_token, pos_embed_3d), dim=1)  # Shape: (1, num_patches_3d + 1, embed_dim)
...
def run_one_image(img, model, save_dir, NORM, mask_ratio=0.75):
    ...
    # visualize the mask
    mask = mask.detach()
    mask = mask.unsqueeze(-1).repeat(1, 1, model.patch_embed.patch_size[0]*model.patch_embed.patch_size[1] ) 
...
def run_two_volume(img, img_future, model, save_dir, NORM, mask_ratio=0.75):
    ...
    # run MAE
    *_, y, mask = model(x, x_future, mask_ratio=mask_ratio)
    y = model.unpatchify3D(y)
    y = y.detach().cpu()
    x = x.detach().cpu()
    x_future = x_future.detach().cpu()
...
def val_saver(valloader, model, save_dir, epoch, NORM=None, mask_ratio=0.75, single=True, volume=False):
    ...
    with torch.no_grad():
        for i in valloader:
            save_path = save_dir+'/'+str(epoch)+'/'+str(i["patID"][0])+'_'+str(i['visitID'][0])+'_'+str(i['visitID_1'][0])
            if volume:
...
# layer decay now works! 19.11.2024
def param_groups_lrd(model, weight_decay=0.05, no_weight_decay_list=[], layer_decay=1.0, large_lr_list=[], lr=1e-5):
    """
    Parameter groups for layer-wise lr decay
    Following BEiT: https://github.com/microsoft/unilm/blob/master/beit/optim_factory.py#L58
    Fixed for standard pytorch scheduler
    large_lr_hist is introduced for classification head with x10 lr
    """
    param_group_names = {}
    param_groups = {}
