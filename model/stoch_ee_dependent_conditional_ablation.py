...
class stoch_ee_dep_cond_ablation(nn.Module):
    ...
    def __init__(self, img_size=(224,224,32), patch_size=(16,16,16), in_chans=3,
        ...
        self.decoder_time_embed = TimestepEmbedder(decoder_embed_dim, 64, 160) # This is added to sampled CLS Q in the decoder #TODO not used for now
    ...
    def forward_decoder(self, z, x_future, ids_restore, delta_t, return_reg=False):
        """
        ...
        kvx_h = self.decoder_embed_stoch(z)# + self.decoder_pos_embed[:, :self.num_prefix_tokens, :] # NOTE it is just 0s nothing to add
        # embed tokens
        ...
        # add time embeddings to x_future
        #x_future = x_future + self.decoder_time_embed(delta_t).unsqueeze(1)
        #kvx_h = kvx_h + self.decoder_time_embed(delta_t).unsqueeze(1)
    ...
    @torch.no_grad()
    def forward_prior(self, imgs, delta_t):
        """
        ...
        pred = self.forward_decoder(h, prior_z, delta_t=delta_t)
        return pred
    ...
    def forward(self, imgs, imgs_future, delta_t, mask_ratio=0.75, return_reg=False, return_indv_loss=False):
        ...
        # Stochastic loss
        future_pred = self.forward_decoder(post_z, h_future, ids_restore, delta_t)
...
def stoch_ee_dep_cond_ablation_mae3d_vit_base_patch16_dec384d8b(patch_size=(16,16,8), **kwargs):
    model = stoch_ee_dep_cond_ablation(
...
stoch_ee_dep_cond_ablation_mae3d_vit_base_patch16 = stoch_ee_dep_cond_ablation_mae3d_vit_base_patch16_dec384d8b  # decoder: 384 dim, 8 blocks