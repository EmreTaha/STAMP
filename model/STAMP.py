...
from .vit_helpers import get_3d_sincos_pos_embed, PatchEmbed3D, TimestepEmbedder, CrossAttention
...
class STAMP(nn.Module):
    ...
    """
    def __init__(self, img_size=(224,224,32), patch_size=(16,16,16), in_chans=1,
                 ...
                 mask_all=False, prior_dist='straight', stoch_mode='sample', pre_ln=False):
        super().__init__()
        ...
        self.encoder_time_embed = TimestepEmbedder(embed_dim, 64, 160) # This is added to past embeddings at the beginning of the encoder
        ...
        # Prior only takes src_h, but it is conditioned on delta t
        ...
        self.decoder_time_embed = TimestepEmbedder(decoder_embed_dim, 64, 160) # This is added to sampled CLS Q in the decoder
        ...
        self.pre_ln = pre_ln # Whether to create distribution after the norm layer
    ...
    def initialize_weights(self):
        ...
        nn.init.normal_(self.decoder_time_embed.mlp[0].weight, std=0.02)
        nn.init.normal_(self.decoder_time_embed.mlp[2].weight, std=0.02)

        nn.init.normal_(self.encoder_time_embed.mlp[0].weight, std=0.02)
        nn.init.normal_(self.encoder_time_embed.mlp[2].weight, std=0.02)
    ...
    def perturb(self, x):
        """
        From RSP, add noise to the input. It doesnt work, random horizontal flip is better
        """
        noise = torch.randn_like(x) * self.noise_scale
        
        return x + noise

    def forward_encoder(self, x, x_future, mask_ratio, delta_t):
        # embed patches
        ...
        # add time embeddings to x
        # Only cls is conditioned
    ...
    def forward_decoder(self, x, z, x_future, ids_restore, delta_t, return_reg=False):
        """
        ...
        delta_t: time between x and x_future. Should be a tensor list of integers with dtype float
        ids_restore: [N, L], restore indices
        """
        if self.discrete != 0:
            z = z.reshape(*z.shape[:-2], 1, self.stoch * self.discrete)
        z = self.decoder_embed_stoch(z)# + self.decoder_pos_embed[:, :self.num_prefix_tokens, :] # NOTE it is just 0s nothing to add
        h = self.decoder_embed(x)
        h[:, :self.num_prefix_tokens, :] = h[:, :self.num_prefix_tokens, :] + self.decoder_time_embed(delta_t).unsqueeze(1) # Add time embed to cls
        kvx_h = torch.cat([z, h], dim=1) # Concat deterministic and stochastic features
    ...
    def alternative_kl_loss(self, post_logits, prior_logits):
        """
        Symmetric KL divergence loss. The places are not swapped, but SG operation applied once to prior, once to post
        """
        balance = self.kl_balance
        ...
        post_to_prior_kl = torch.clip(post_to_prior_kl, min=freebit)
        prior_to_post_kl = torch.clip(prior_to_post_kl, min=freebit)

        kl_loss = (
            post_to_prior_kl * balance + prior_to_post_kl * (1.0 - balance)
        ).mean()

        return kl_loss, kl_loss, [post_to_prior_kl.detach().mean(), prior_to_post_kl.detach().mean()]

    @torch.no_grad()
    def get_embedding(self, x, delta_t, norm=True):
        # embed patches
        x = self.patch_embed(x)
    ...
    @torch.no_grad()
    def forward_prior(self, imgs, delta_t, norm=True):
        """
        Sample from prior and generate
        """
        h = self.get_embedding(imgs,delta_t,norm)
        prior_h = h[:, 0]
    ...
    @torch.no_grad()
    def forward_decoder_nomask(self,x,z,delta_t):
        """
        Predict future from h_t + z_t+k. For querry it only uses the mask tokens
        x: [N, L, D], deterministic features
        z: [N, L, M], stochastic features
    ...
    @torch.no_grad()
    def reconstruct_prior(self, imgs, delta_t, norm=True):
        """
        Sample from prior and generate
        """
        h = self.get_embedding(imgs,delta_t,norm)
        ...
        pred = self.forward_decoder_nomask(h, prior_z, delta_t=delta_t)
        return pred

    def forward(self, imgs, imgs_future, delta_t, mask_ratio=0.75, return_reg=False, return_indv_loss=False):
        h, h_future, mask, ids_restore = self.forward_encoder(imgs, imgs_future, mask_ratio=mask_ratio, delta_t=delta_t)

        # Posterior distribution from both images q(z_{t+delta_t}|h_t, h^M_{t+delta_t})
        post_h = torch.cat([h[:, 0], h_future[:, 0]], -1) # Use CLS tokens
        post_logits = self.to_posterior(post_h) # Used in KL
        post_dist = self.make_dist(post_logits)
        if self.stoch_mode == 'sample':
            post_z = post_dist.rsample() # Used in decoder
        elif self.stoch_mode == 'mode':
            post_z = post_dist.mode
        elif self.stoch_mode == 'mean':
            post_z = post_dist.mean
        else:
            raise ValueError('stoch_mode should be either sample, mode or mean')

        # Prior distribution only from current images p(z_{t+delta_t}|h_t,delta_t)
        prior_h = h[:, 0] # Use CLS tokens + time embedding 
        prior_logits = self.to_prior(prior_h) # Used in KL

        # If pre_ln is True, norm is applied after the distribution
        if self.pre_ln:
            h = self.norm(h)
            h_future = self.norm(h_future)

        # Stochastic loss
        future_pred = self.forward_decoder(h, post_z, h_future, ids_restore, delta_t)
        ...
        kl_loss, kl_value, kl_indv = self.kl_loss(post_logits, prior_logits) # KL makes them distro separately because detach is used
...
def stamp_vit_base_patch16_dec384d8b(patch_size=(16,16,8), **kwargs):
    model = STAMP(
...
stamp_vit_base = stamp_vit_base_patch16_dec384d8b  # decoder: 384 dim, 8 blocks