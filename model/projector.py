...
class MLP(nn.Module):
    ...
    # Update 2: bias term now controls everything, use bias last
    # Update 3: update 2 is canceled
    def __init__(self, in_dim: int,
                 hidden_dims: Union[int, tuple],
                 bias = True, norm_last = False, bias_last = False, #bias_last = True,
                 norm_embed=False, norm_func = nn.BatchNorm1d, actv_func = partial(nn.ReLU,inplace=True)):
        super().__init__()
        ...
        # It attaches the last layer with spectral or as it is, spectral is a wrapper
        if norm_last=="SN":
        ...
        if norm_last=='BN':
            ...
            mlp.append(nn.GroupNorm(32, hidden_dims[-1], affine=False))
        elif norm_last=='GN_affine':
            mlp.append(nn.GroupNorm(32, hidden_dims[-1], affine=True))
        elif norm_last=='SoftPlus':
            mlp.append(nn.Softplus()) # Default beta is 1, which is the regularization loss term
