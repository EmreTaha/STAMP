...
class ind_cond_ablation(nn.Module):
    ...
    def __init__(self, img_size=(224,224,32), patch_size=(16,16,16), in_chans=3,
        ...
        self.decoder_time_embed = TimestepEmbedder(decoder_embed_dim, 64, 160) # This is added to Q in the decoder
    ...
    def vis_forward(self, imgs, imgs_future, delta_t, ids_restore, return_reg=False):
        h, h_future = self.vis_forward_encoder(imgs, imgs_future,  ids_restore, delta_t)
        # Posterior distribution from both images
        # TODO is just using CLS enough maybe use average
        post_h = torch.cat([h[:, 0], h_future[:, 0]], -1) # Use CLS tokens
        post_logits = self.to_posterior(post_h)
        ...
        #prior_dist = self.make_dist(prior_logits)
        #prior_z = prior_dist.rsample()
        pred = self.forward_decoder(h, post_z, h_future, ids_restore, delta_t)
        return pred



def ind_cond_ablation_mae3d_vit_base_patch16_dec384d8b(patch_size=(16,16,8), **kwargs):
    model = ind_cond_ablation(
...
ind_cond_ablation_mae3d_vit_base_patch16 = ind_cond_ablation_mae3d_vit_base_patch16_dec384d8b  # decoder: 384 dim, 8 blocks