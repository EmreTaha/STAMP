...
from .vit_helpers import get_3d_sincos_pos_embed, PatchEmbed3D, CrossAttention, TimestepEmbedder
...
class ce_SiamMaskedAutoencoderViT3D(nn.Module):
    ...
    def __init__(self, img_size=(224,224,32), patch_size=(16,16,16), in_chans=3,
        ...
        self.encoder_time_embed = TimestepEmbedder(embed_dim, 64, 160)
    ...
    def forward_encoder(self, x, x_future, mask_ratio, delta_t):
        ...
        #x_future = x_future + self.time_embed(delta_t).unsqueeze(1)
        ...
        x = x + self.encoder_time_embed(delta_t).unsqueeze(1)
    ...
    def forward_decoder(self, x, x_future, ids_restore, delta_t, return_reg=False):
        '''
        x: current frame
        x_future: future frame
        ids_restore: indices to restore the shuffled patches
        delta_t: time between x and x_future. Should be a tensor list of integers with dtype float
        return_reg: return register tokens
        '''
        ...
        # add pos embed
        x = x + self.decoder_pos_embed # K,V
        x_future = x_future + self.decoder_pos_embed # Q
    ...
    def forward(self, imgs, imgs_future, delta_t, mask_ratio=0.75, return_reg=False):
        latent, latent_future, mask, ids_restore = self.forward_encoder(imgs, imgs_future, mask_ratio, delta_t)
        pred = self.forward_decoder(latent, latent_future, ids_restore, delta_t, return_reg)  # [N, L, p*p*1]
    ...
    # NOTE from here only for visualization
    def det_masking(self, x,ids_restore):
        """
        Perform per-sample random masking by per-sample shuffling.
        Per-sample shuffling is done by argsort random noise.
        x: [N, L, D], sequence
        ...
        # sort noise for each sample
        ids_shuffle_recovered = torch.argsort(ids_restore, dim=1)

        # keep the first subset
        ids_keep = ids_shuffle_recovered[:, :len_keep]
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))

        return x_masked

    def vis_forward_encoder(self, x, x_future, ids_restore, delta_t):
        '''
        x: current frame
        x_future: future frame
        mask_ratio: ratio of patches to mask
        ...
        # mask only future: length -> length * mask_ratio
        x_future = self.det_masking(x_future, ids_restore)
    ...
    def vis_forward(self, imgs, imgs_future, delta_t, ids_restore, return_reg=False):
        latent, latent_future = self.vis_forward_encoder(imgs, imgs_future, ids_restore, delta_t)
        pred = self.forward_decoder(latent, latent_future, ids_restore, delta_t, return_reg)  # [N, L, p*p*1]
        return pred
...
def ce_siam_mae3d_vit_base_patch16_dec384d8b(patch_size=(16,16,8), **kwargs):
    model = ce_SiamMaskedAutoencoderViT3D(
...
ce_siam_mae3d_vit_base_patch16 = ce_siam_mae3d_vit_base_patch16_dec384d8b  # decoder: 384 dim, 8 blocks
